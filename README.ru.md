...
- [Установка](#установка)
- [Использование с помощью CLI](#использование-с-помощью-CLI)
  - [Простой способ](#простой-способ)
  - [Продвинутый способ](#продвинутый-способ)
- [Использование с помощью API](#использование-с-помощью-API)
...
MdPugToHtml массово конвертирует файлы Markdown в HTML. Можно указать шаблон Pug, по которому все страницы Markdown будут преобразованы в HTML. Также, MdPugToHtml генерирует массив объектов, в котором для каждой созданной HTML страницы содержатся следующие данные:

- путь к файлу HTML страницы
- заголовок HTML страницы
- краткое описание HTML страницы
- любые другие данные, которые укажет пользователь в разделе Frontmatter статьи Markdown.
...
Этот способ позволяет, легко отконвертировать весь каталог с файлами Markdown в страницы HTML. И, дополнительно, получить HTML файл со списком ссылок на эти страницы.

Откройте терминал, создайте каталог, например, `my-site`, и перейдите в него:
...
В файле `package.json` настройте MdPugToHtml:
...
Где `content` - это каталог, в котором находятся файлы Markdown. Это значит, что каталог находится в корне этого проекта.

Если файлы Markdown находятся в другом месте укажите иной путь, например:
...
Теперь запустите MdPugToHtml. Для этого из каталога `my-site` в терминале выполните команду:

```
npm run start
```

В результате, будет создан каталог `mpth`, в котором будут страницы HTML, преобразованные из Markdown файлов.

Также, будут созданы три файла:

- `mpth/index.html`, который содержит список ссылок на созданные файлы Html. Если открыть этот файл в браузере, то можно просматривать созданные страницы из браузера переходя по ссылкам.

- `mpth/mpth-data.pug`, который содержит массив объектов. Этот массив может быть использован для создания списка ссылок на статьи в формате блога. Как это сделать смотри [Продвинутый способ](#продвинутый-способ).

- `mpth/mpth-template.pug`, который содержит шаблон Pug. Именно этот шаблон использует конвертер MdPugToHtml для создания страниц HTML, если разрешено конвертирование по шаблону. Вы можете изменить шаблон и снова запустить команду `npm run start`. Статьи будут переформатированы в соответствии в новым шаблоном.
...
- ключ `-o` - путь к каталогу сборки проекта (по умолчанию `mpth`)
- ключ `-t` - путь к каталогу шаблона статьи с именем `index.pug` (по умолчанию `mpth`)
- ключ `-d` - путь к каталогу, куда будет сгенерирован файл `linkList.pug` (по умолчанию `mpth`)
