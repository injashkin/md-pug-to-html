...
Этот способ позволяет, легко отконвертировать весь каталог с файлами Markdown в файлы HTML. И, дополнительно, получить HTML файл со списком ссылок на файлы HTML.
...
Где `/home/my/content` - путь к каталогу, в котором находятся файлы Markdown.
...
В результате, будет создан каталог `mpth`, в котором вы найдете страницы HTML, преобразованные из Markdown файлов.
...
- `mpth/mpth-data.pug`, который содержит массив объектов. Этот массив может быть использован, например, для создания списка ссылок на статьи в формате блога. Как это сделать смотри [Продвинутый способ](#продвинутый-способ).
