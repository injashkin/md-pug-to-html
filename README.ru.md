...
## Самый простой способ использования MdPugToHtml

Чтобы запустить MdPugToHtml, на вашем компьютере должен быть установлен Node.js.

Допустим, у вас есть куча файлов Markdown, и они находятся в каталоге `/home/my/content`. Вы хотите их преобразовать в файлы HTML.
...
Вот и все! Теперь вы можете с помощью файлового менеджера перейти в каталог `/home/my/content`, в котором вы увидите каталог `mpth`. В этом каталоге находятся откомпилированные файлы HTML. Вы можете открыть файл `mpth-articles.html` в браузере, в результате, увидите список ссылок на свои файлы. Переходя по этим ссылкам вы можете просматривать свои документы.

Более продвинутые способы использования MdPugToHtml смотрите ниже.
...
Простой способ приведен в начале этого руководства, здесь будут пояснения к нему.

При выполнении вышеприведенной команды, будет установлен конвертер MdPugToHtml и, затем, будет выполнено преобразование Markdown файлов. Установка конвертера будет произведена только при первом запуске команды. При последующих запусках будет производится только конвертирование. Опция `-y` задана для того, чтобы система не задавала вопроса, нужно ли устанавливать md-pug-to-html. Поэтому, при повторных запусках опцию `-y` можно не применять.

Помимо откомпилированных HTML файлов, в каталоге `mpth` будут находится четыре дополнительных файла:

- `mpth-articles.html` - содержит список ссылок на созданные файлы Html. Если открыть этот файл в браузере, то можно просматривать созданные страницы из браузера переходя по ссылкам. Вы можете отключить создание файла `mpth-articles.html` применив в вышеприведенной команде опцию `-I`.
...
- `mpth-template.pug` - шаблон Pug. Именно этот шаблон используется конвертером MdPugToHtml для создания страниц HTML, если разрешено конвертирование по шаблону. Отключить конвертирование по шаблону можно опцией `-n`. Тогда конвертирование будет производиться напрямую без применения шаблона. Вы можете изменить шаблон и снова запустить команду `npx md-pug-to-html /home/my/content`. Статьи будут переформатированы в соответствии в новым шаблоном. Если вы напортачили в шаблоне и у вас по каким-то причинам не конвертируются файлы, то удалите файл `mpth-template.pug` или весь каталог `mpth` и перезапустите вышеприведенную команду. Файл шаблона будет создан в первоначальном виде.

- `github.css` - файл стилей, благодаря которому ваши документы имеют оформленный вид.
...
Опции:
  ...
  -O, --obj <str|path>  объект или файл с опциями в формате JSON/JavaScript
  ...
  -s, --styles <name>   название темы (default: "github")
...
- `styles` - имя встроенного файла стилей (по умолчанию `styles: 'github'`). Можно отключить встроенные стили так `styles: 'no'`
