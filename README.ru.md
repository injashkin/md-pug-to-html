...
<details>
  ...
  <ol>
    ...
    <li>
      <a href="#использование-с-помощью-CLI">Использование с помощью CLI</a>
      <ul>
        <li><a href="#простой-способ">Простой способ</a></li>
        <li><a href="#продвинутый-способ">Продвинутый способ</a></li>
      </ul>
    </li>
...
Эти страницы сохраняются в указанном каталоге с сохранением полной структуры исходного каталога (с Markdown файлами). Файлы Markdown могут содержать Frontmatter[1] данные. Эти данные будут использованы как метаданные HTML страниц.
...
Этот способ позволяет, указав всего лишь каталог с файлами Markdown, переконвертировать эти файлы в страницы HTML. И, дополнительно, получить файл со списком отконвертированных файлов, а также файл шаблона Pug, на основе которого была выполнена конвертация всех файлов Markdown в файлы HTML.
...
В файле `package.json` через ключ `-i` укажите каталог, где находятся файлы Markdown:
...
Здесь указан каталог `content`. Это значит, что он должен находится в одном каталоге с файлом `package.json`. Если файлы Markdown находятся в другом месте укажите иной путь, например:
...
Из каталога `my-site` в терминале выполните команду:

```
npm run start
```

В результате, будет создан каталог `docs`, в котором будут страницы HTML, преобразованные из Markdown файлов.

Также, будут созданы два файла, которые, возможно, будут вам полезны:

- файл `src/data/link-list.pug`, который будет содержать массив объектов. Этот массив может быть использован для создания списка ссылок на статьи в формате блога. Как это сделать смотри [Продвинутый способ](#продвинутый-способ).

- файл `src/article/index.pug` с шаблоном Pug, по которому отформатированы все страницы HTML. Вы можете изменить шаблон и снова перезапустить команду `npm run start`. Статьи будут переформатированы в соответствии в новым шаблоном.
...
- ключ `-t` - путь к каталогу шаблона статьи с именем `index.pug` (по умолчанию `src/article`)
