...
<h3 align="center">MdPugToHtml</h3>
  ...
  <p align="center">
    Массово компилирует HTML страницы из файлов Markdown с применением Pug шаблона.
    <br />
...
MdPugToHtml в указанном каталоге обходит рекурсивно все подкаталоги и находит файлы Markdown, конвертируя их в страницы HTML в соответствии с указанным шаблоном Pug. Затем помещает отконвертированные страницы в указанный выходной каталог. В выходном каталоге полностью сохраняется структура исходного каталога.
...
Также, будут созданы два файла:

- файл `src/data/link-list.pug` содержит массив объектов. Этот массив может быть использован для создания списка ссылок на статьи в формате блога. Как это сделать смотри [Продвинутый способ](#продвинутый-способ).

- файл `src/article/index.pug` содержит шаблон Pug. Именно этот шаблон использует конвертер MdPugToHtml для создания страниц HTML. Вы можете изменить шаблон и снова запустить команду `npm run start`. Статьи будут переформатированы в соответствии в новым шаблоном.
